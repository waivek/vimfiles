...
command! -nargs=1 FindPersonalFunctions call  s:FindPersonalFunctions(<q-args>)
...
function! introspect#GetSid(path)
    if !exists("s:script_D") || len(s:script_D) == 0
...
function! s:JumpToCommand()
    ...
    call common#PrintDict(dictionaries[0])
    return dictionaries
endfunction
